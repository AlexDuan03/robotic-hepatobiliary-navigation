...
def build_model(img_size: int, num_classes: int, device: torch.device):
...
def load_model(checkpoint_path: str, img_size: int, num_classes: int, device: torch.device):
    model = build_model(img_size, num_classes, device)
...
def main():
    ...
    parser.add_argument("--checkpoint", type=str, default="best_model_transunet_4class_dice.pth")
    parser.add_argument("--image", type=str, default=None, help="单张图片路径；不填就随机选")
    parser.add_argument("--test_dir", type=str, default="dataset_4class_v2/test/images")
    parser.add_argument("--output_dir", type=str, default="predict_results/transunet_v2")
